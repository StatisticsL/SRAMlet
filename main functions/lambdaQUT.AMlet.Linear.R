lambdaQUT.AMlet.Linear<-function(X,sigma,filter.number=1,family="DaubExPhase", bc="periodic",
  ...
  lambda.QUT.S=lambdaQUT.AMlet(X=X,sigma=sigma,filter.number=filter.number,family=family, bc=bc,
  ...
  if(type=='MC'){
    ...
    num2=t(Xl2*lambda.QUT.S/lambdas.QUT.linear)%*%z
    lambdas2=apply(abs(num2),2,max)
