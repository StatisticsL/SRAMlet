lambdaQUT.AMlet.Haar.Linear<-function(X,sigma,filter.number=1,family="DaubExPhase", bc="periodic",
  ...
  lambda.QUT.HS=lambdaQUT.AMlet.Haar(X=X,sigma=sigma,filter.number=filter.number,family=family, bc=bc,
  ...
  if(type=='MC'){
    ...
    num2=t(Xl2)%*%z
    lambdas2=apply(abs(num2),2,max)
    lambdas.QUT.linear=quantile(lambdas2,1-alpha)
    num2=t(Xl2*lambda.QUT.HS/lambdas.QUT.linear)%*%z
    lambdas2=apply(abs(num2),2,max)
    lambdas3=apply(z,2,numfct,Xorder=Xorder,filter.number=1,family="DaubExPhase", bc=bc)
