AMlet.QUT.Mad<-function(y,X,lambda=NA,sigma=NA,p0=1,filter.number=1,family="DaubExPhase",bc="periodic",
  ...
  if(P>1){
    if(is.na(lambda)&is.na(sigma)){
      lambda0=lambdaQUT.AMlet(X=X,sigma=1,filter.number=filter.number,family=family, bc=bc,
