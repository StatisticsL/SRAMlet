AMlet.Universal.Mad<-function(y,X,lambda=NA,sigma=NA,p0=1,filter.number=1,family="DaubExPhase",bc="periodic",
  ...
  if(P>1){
    if(is.na(lambda)&is.na(sigma)){
      ...
      for (p_index in 1:P) {
        ...
        outp=softwavethresh(y=resp,as.matrix(X[,p_index]),lambda=NA,lambdatype='Universal',sigma=NA,p0=p0,pen.father=T,
      ...
      if(is.na(lambda)){lambda=sigma*sqrt(2*log(N))}
      cc=mean(y)
      muhat=matrix(0, N, P)
      alpha.hat=matrix(0,N, P)
      res=y-rowSums(muhat)-cc
