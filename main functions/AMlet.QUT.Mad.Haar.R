AMlet.QUT.Mad.Haar<-function(y,X,lambda=NA,sigma=NA,p0=1,filter.number=1,family="DaubExPhase",bc="periodic",
  ...
  if(P>1){
    if(is.na(lambda)&is.na(sigma)){
      lambda0=lambdaQUT.AMlet.Haar(X=X,sigma=1,filter.number=filter.number,family=family, bc=bc,
      ...
      while (continue&(iter<max.iter)) {
        ...
        for (p_index in indexes[-index.deleted]) {
          ...
          if(p_index<= P){
            ...
            outp=softwavethresh(y=resp,as.matrix(X[,(p_index-P)]),lambda=lambda,sigma=sigma,p0=p0,pen.father=T,
                                filter.number=filter.number,family=family,bc=bc,
                                alpha=alpha,M_MC=M_MC,M_GEV=M_GEV,type=type,com.sure=F,com.sqrtrss=com.sqrtrss)
          }
      ...
      if(is.na(lambda)){lambda=lambdaQUT.AMlet(X=X,sigma=sigma,filter.number=filter.number,family=family, bc=bc,
                                               alpha=alpha,M_MC=M_MC,M_GEV=M_GEV,type=type)}
      cc=mean(y)
      muhat=matrix(0, N, 2*P)
      alpha.hat=matrix(0,N, 2*P)
