...
for (simu in 1:num_simulations){
  ...
  ####----sqrtAMlet----------------------------------
  fit_sqrtAMlet=SRAMlet(y=y.train,X=x.train,lambda=NA,p0=p0,addHaar=F,addLinear=F,filter.number=filter.number,family=family,
                        alpha= alpha,M_MC=M_MC,M_GEV=M_GEV,type=type)###haar better than without haar
  ...
  ####-----mad AMlet-------------------------------------
  fit_madAMlet=AMlet(y=y.train,X=x.train,lambda=NA,sigma=NA,p0=p0,filter.number=filter.number,family=family,
...
save(mse,modelsize,results,selectedindex.SRAMlet,selectedindex.madAMlet,selectedindex.HAM,selectedindex.SAM
     ,selectedindex.mgcv,file = './simulation_realdata/results-glucose.RData')
save(fitResults.sqrtAMlet,fitResults.madAMlet,fitResults.HAM,fitResults.SAM,fitResults.mgcv,
     file ='./simulation_realdata/fitting-glucose.RData')
